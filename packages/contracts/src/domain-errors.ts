...
// Классы вынесены в `domain-errors-delivery.ts` по max-lines (L2 линтер) при слиянии
// `feat/ep-11-returns-flow` — реэкспортируется напрямую из `index.ts` (не отсюда, циклический
// импорт), тот же приём, что `domain-errors-inventory.ts`/`domain-errors-support.ts`.
// `CourierTenantMismatchError`/`CourierNotEligibleError` (SRS-DOM-037/038) остаются здесь —
// переиспользуются этим модулем без изменений (§A.9 «уже определённые коды»).
