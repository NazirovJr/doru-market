...
> Прочитай этот файл целиком, до строки `=== КОНЕЦ ЗАДАНИЯ DTJ-307d ===` в самом низу.
> Нет этой строки — файл обрезан: ничего не делай и ответь одной фразой «ЗАДАНИЕ ОБРЕЗАНО».
...
8. git: можно `git status`, `git diff`, создать свою ветку и закоммитить в неё СВОЮ работу
   (команды — в разделах 0 и 8). Нельзя: `merge`, `rebase`, `push`, `reset`, `stash`, переключаться
   на чужие ветки, коммитить файлы вне списка раздела 0. Ветку в `development` вливает координатор.
...
1. Запусти гейт с `-Full` (добавь `-Full` в команду из раздела 1). Нужно `GATE: PASS`.
   Допустимое исключение (по данным `pnpm verify` от 19.09.2026): шаг
   `vitest @dorutj/worker (весь пакет, с порогами покрытия)`, если в его выводе единственная ошибка —
   строка вида `Coverage for functions (…%) does not meet global threshold (…%)`. Любая другая
   ошибка в этом шаге — твоя.
2. Закоммить работу в свою ветку: `git add` — только файлы из раздела 0, затем
   `git commit -m "feat(worker): DTJ-307d — консьюмер очереди picking-sla-watchdog"`.
   Сообщение — ровно это, без приписок об ИИ-авторстве. Не мержи и не пушь.
3. Выполни `git status --short` (после коммита должно быть пусто) и `git log --oneline -1`.
...
ФАЙЛЫ:
<вывод git show --stat --oneline HEAD>
ГЕЙТ:
...
Не пиши «успешно», «всё работает», «соответствует стилю». Только факты и дословный вывод команд.

5. Если работа запущена как цель (`/goal`), заверши её: вызови `get_goal`, затем `update_goal` с его
   `goal_id` и `revision` — `action: complete` при DONE, `action: blocked` и `blocked_reason` при BLOCKED.
