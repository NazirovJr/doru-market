...
> Прочитай этот файл целиком, до строки `=== КОНЕЦ ЗАДАНИЯ DTJ-307a ===` в самом низу.
...
1. Запусти гейт с `-Full` (добавь `-Full` в команду из раздела 1). Нужно `GATE: PASS`.
   Допустимое исключение: исключений нет (`apps/worker` в этом задании не меняется).
2. Закоммить работу в свою ветку: `git add` — только файлы из раздела 0, затем
   `git commit -m "feat(orders): DTJ-307a — SlaBreachedEvent и internal-эндпоинт мягкого нарушения SLA сборки"`.
