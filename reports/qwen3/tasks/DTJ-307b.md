...
> Прочитай этот файл целиком, до строки `=== КОНЕЦ ЗАДАНИЯ DTJ-307b ===` в самом низу.
...
1. Запусти гейт с `-Full` (добавь `-Full` в команду из раздела 1). Нужно `GATE: PASS`.
   Допустимое исключение: исключений нет. Гейт пропустит `test-app.ts` как интеграционный — это нормально.
2. Закоммить работу в свою ветку: `git add` — только файлы из раздела 0, затем
   `git commit -m "feat(orders): DTJ-307b — getPickupSlaBufferMinutes в TenancyFacadePort"`.
