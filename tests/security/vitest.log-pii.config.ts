/**
 * Отдельный vitest-конфиг ТОЛЬКО для `log-pii-leakage.spec.ts` (DTJ-425) — см. подробное
 * обоснование в JSDoc `vitest.config.ts` (`exclude`): этот файл живым прогоном обнаружено ловит
 * утечку состояния (`LOG_LEVEL`), когда исполняется В ОДНОМ vitest-воркере ПОСЛЕ другого файла
 * пакета (`tenant-isolation-idor.spec.ts`), уже транзитивно импортировавшего `@/config/...`.
 * Полностью отдельный процесс (`pnpm run test:log-pii`, вызываемый третьим шагом `test`)
 * устраняет утечку гарантированно, а не полагается на `isolate`-поведение пула. Значения
 * `test.env` — 1:1 копия `vitest.config.ts` (см. её файл для обоснования каждого поля) —
 * `LOG_LEVEL` этот файл переопределяет сам, прямым присваиванием, перед своим `import('@/main.js')`.
 */
...
export default defineConfig({
  ...
  test: {
    ...
    pool: 'vmThreads',
    include: ['log-pii-leakage.spec.ts'],
    env: {
      NODE_ENV: 'test',
      ...
      OTP_RATE_LIMIT_KEY_PREFIX: 'test:otp_rl_security_logpii',
      ...
      JWT_KID: 'test-v1-security-logpii',
