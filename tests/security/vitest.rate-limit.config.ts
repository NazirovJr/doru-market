/**
 * Отдельный vitest-конфиг ТОЛЬКО для `rate-limiting.spec.ts` (DTJ-425) — см. подробное
 * обоснование в JSDoc `vitest.config.ts` (`exclude`): этот файл живым прогоном обнаружено
 * ловит утечку состояния (`RATE_LIMIT_ANON_PER_MIN`), когда исполняется В ОДНОМ vitest-воркере
 * с другими файлами пакета. Полностью отдельный процесс (`pnpm run test:rate-limit`,
 * вызываемый вторым шагом `test`) устраняет утечку гарантированно, а не полагается на
 * `isolate`-поведение пула. Значения `test.env` — 1:1 копия `vitest.config.ts` (см. её файл для
 * обоснования каждого поля), кроме `RATE_LIMIT_ANON_PER_MIN`/`RATE_LIMIT_USER_PER_MIN` — здесь
 * не заданы вовсе, `rate-limiting.spec.ts` сам управляет ими напрямую перед `import('@/main.js')`.
 */
...
export default defineConfig({
  ...
  test: {
    ...
    env: {
      ...
      OTP_RATE_LIMIT_KEY_PREFIX: 'test:otp_rl_security_ratelimit',
      ...
      JWT_KID: 'test-v1-security-ratelimit',
