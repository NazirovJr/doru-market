...
сумма отличается от суммы, которую клиент ПОДТВЕРДИЛ на экране, Then эта ГРУППА (аптека) не
...
**Поправка CTO (волна 6): подтверждённая сумма передаётся ПО ГРУППАМ, а не одним числом на
запрос.** Исходная редакция объявляла одно поле `expectedTotalDiram` на весь запрос, но
последствие описывала пер-групповое («эта ГРУППА не оформляется»). Как одно число сверяется с
несколькими группами, не определялось, и на практике это делало защиту неприменимой в главном
сценарии продукта: при корзине из двух и более аптек любое переданное значение совпало бы
максимум с одной группой и дало бы ложный `PRICE_OR_STOCK_CHANGED` всем остальным. Пробел
независимо обнаружили два исполнителя (DTJ-231 задокументировал ограничение в JSDoc сервиса,
DTJ-235 отказался отправлять поле, чтобы не порождать ложные отказы) — то есть в R1 защита
фактически не работала ни в каком виде.

Корректная форма: клиент передаёт ожидаемую сумму ДЛЯ КАЖДОЙ аптеки, ключом служит `pharmacyId`;
сверка идёт с суммой соответствующей группы. Группы, для которых ожидание не передано,
оформляются без проверки дрейфа (поле остаётся опциональным). Сравнивать следует сумму ПОЗИЦИЙ
(`items_total`), а не итог с доставкой: стоимость доставки считается сервером и пользователю на
момент подтверждения неизвестна, поэтому требовать от клиента её угадывания — значит сделать
проверку невыполнимой по построению.
