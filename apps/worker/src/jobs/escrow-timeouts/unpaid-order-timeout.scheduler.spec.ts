/**
 * Тест `UnpaidOrderTimeoutScheduler` (DTJ-253). Планировщик — тонкая обёртка над BullMQ `Worker`/`Queue`;
...
import { UNPAID_ORDER_TIMEOUT_JOB_NAME, UNPAID_ORDER_TIMEOUT_SCHEDULER_ID, UNPAID_ORDER_TIMEOUT_TZ } from './unpaid-order-timeout.constants.js'
import type { UnpaidOrderTimeoutJob } from './unpaid-order-timeout.job.js'
import { UnpaidOrderTimeoutScheduler, type UnpaidOrderTimeoutScheduleConfig } from './unpaid-order-timeout.scheduler.js'
...
describe('UnpaidOrderTimeoutScheduler', () => {
  let queueMock: { name: string; upsertJobScheduler: Mock }
  let connection: Redis
  let runOnceMock: Mock
  let job: UnpaidOrderTimeoutJob
  let scheduler: UnpaidOrderTimeoutScheduler

  beforeEach(() => {
    vi.clearAllMocks()
    fakeWorkerClose.mockClear()
    queueMock = { name: 'unpaid-order-timeout', upsertJobScheduler: vi.fn().mockResolvedValue(undefined) }
    connection = {} as Redis
    runOnceMock = vi.fn().mockResolvedValue(undefined)
    job = { runOnce: runOnceMock } as unknown as UnpaidOrderTimeoutJob
    const schedule = { queue: queueMock as unknown as Queue, cron: CRON } as UnpaidOrderTimeoutScheduleConfig
    scheduler = new UnpaidOrderTimeoutScheduler(schedule, connection, job)
  })
  ...
  it('onModuleInit регистрирует расписание с cron из конфига и ожидаемыми идентификаторами', () => {
    ...
    expect(queueMock.upsertJobScheduler).toHaveBeenCalledWith(
      ...
      { pattern: CRON, tz: UNPAID_ORDER_TIMEOUT_TZ },
      { name: UNPAID_ORDER_TIMEOUT_JOB_NAME },
    )
  })

  it('тик Worker вызывает UnpaidOrderTimeoutJob.runOnce()', async () => {
    scheduler.onModuleInit()
