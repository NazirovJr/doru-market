/**
 * Тест `PayoutExecutionScheduler` (DTJ-250). Планировщик — тонкая обёртка над BullMQ `Worker`/`Queue`;
...
import { PAYOUT_EXECUTION_JOB_NAME, PAYOUT_EXECUTION_SCHEDULER_ID, PAYOUT_EXECUTION_TZ } from './payout-execution.constants.js'
import type { PayoutExecutionJob } from './payout-execution.job.js'
import { PayoutExecutionScheduler, type PayoutExecutionScheduleConfig } from './payout-execution.scheduler.js'
...
describe('PayoutExecutionScheduler', () => {
  let queueMock: { name: string; upsertJobScheduler: Mock }
  let connection: Redis
  let runOnceMock: Mock
  let job: PayoutExecutionJob
  let scheduler: PayoutExecutionScheduler

  beforeEach(() => {
    vi.clearAllMocks()
    fakeWorkerClose.mockClear()
    queueMock = { name: 'payout-execution', upsertJobScheduler: vi.fn().mockResolvedValue(undefined) }
    connection = {} as Redis
    runOnceMock = vi.fn().mockResolvedValue(undefined)
    job = { runOnce: runOnceMock } as unknown as PayoutExecutionJob
    const schedule = { queue: queueMock as unknown as Queue, cron: CRON } as PayoutExecutionScheduleConfig
    scheduler = new PayoutExecutionScheduler(schedule, connection, job)
  })
  ...
  it('onModuleInit регистрирует расписание с cron из конфига и ожидаемыми идентификаторами', () => {
    ...
    expect(queueMock.upsertJobScheduler).toHaveBeenCalledWith(
      ...
      { pattern: CRON, tz: PAYOUT_EXECUTION_TZ },
      { name: PAYOUT_EXECUTION_JOB_NAME },
    )
  })

  it('тик Worker вызывает PayoutExecutionJob.runOnce()', async () => {
    scheduler.onModuleInit()
