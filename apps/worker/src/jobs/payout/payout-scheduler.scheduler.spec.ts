/**
 * Тест `PayoutSchedulerScheduler` (DTJ-249). Планировщик — тонкая обёртка над BullMQ `Worker`/`Queue`;
...
import { PAYOUT_SCHEDULER_JOB_NAME, PAYOUT_SCHEDULER_SCHEDULER_ID, PAYOUT_SCHEDULER_TZ } from './payout-scheduler.constants.js'
import type { PayoutSchedulerJob } from './payout-scheduler.job.js'
import { PayoutSchedulerScheduler, type PayoutSchedulerScheduleConfig } from './payout-scheduler.scheduler.js'
...
describe('PayoutSchedulerScheduler', () => {
  let queueMock: { name: string; upsertJobScheduler: Mock }
  let connection: Redis
  let runOnceMock: Mock
  let job: PayoutSchedulerJob
  let scheduler: PayoutSchedulerScheduler

  beforeEach(() => {
    vi.clearAllMocks()
    fakeWorkerClose.mockClear()
    queueMock = { name: 'payout-scheduler', upsertJobScheduler: vi.fn().mockResolvedValue(undefined) }
    connection = {} as Redis
    runOnceMock = vi.fn().mockResolvedValue(undefined)
    job = { runOnce: runOnceMock } as unknown as PayoutSchedulerJob
    const schedule = { queue: queueMock as unknown as Queue, cron: CRON } as PayoutSchedulerScheduleConfig
    scheduler = new PayoutSchedulerScheduler(schedule, connection, job)
  })
  ...
  it('onModuleInit регистрирует расписание с cron из конфига и ожидаемыми идентификаторами', () => {
    ...
    expect(queueMock.upsertJobScheduler).toHaveBeenCalledWith(
      ...
      { pattern: CRON, tz: PAYOUT_SCHEDULER_TZ },
      { name: PAYOUT_SCHEDULER_JOB_NAME },
    )
  })

  it('тик Worker вызывает PayoutSchedulerJob.runOnce()', async () => {
    scheduler.onModuleInit()
