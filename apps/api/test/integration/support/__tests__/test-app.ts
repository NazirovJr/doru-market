/**
 * `test-app.ts` (EP-14, DTJ-282) — Supertest-harness `SupportTicketsController` против РЕАЛЬНЫХ
 * Postgres/Redis. Тот же приём, что `test/integration/orders/__tests__/test-app.ts` (DTJ-226) —
 * КАЖДЫЙ модуль несёт СВОЮ копию harness'а (не общий файл, установленная конвенция — три
 * прецедента: `auth`/`orders`/`payments`), `Test.createTestingModule` строит дерево из
 * `SupportModule` + разделяемой инфраструктуры, не из полного `AppModule`.
 *
 * `installDefaultTenantContextHook` — ОБЯЗАТЕЛЕН здесь (не опционален): `AuthGuard.
 * assertCrossTenantAccess` (`modules/auth/presentation/guards/auth.guard.ts`) бросает
 * `500 INTERNAL_ERROR` для ЛЮБОЙ роли, кроме `super_admin`, если `TenantContext.get()` не
 * инициализирован — этот harness НЕ поднимает полный `AppModule`, значит
 * `TenantResolutionMiddleware` никогда не срабатывает. Тот же трюк, что auth/orders-harness'ы:
 * резолвит `TenantContext` ИЗ `claims.tenantId` самого токена, минуя реальный middleware.
 */
...
/** Идентична паре `test/integration/orders/__tests__/test-app.ts` — тестовые ключи, НЕ секрет. */
...
const TEST_ENV: Readonly<Record<string, string>> = {
  ...
  OTP_RATE_LIMIT_KEY_PREFIX: 'test:otp_rl_support',
...
export async function createTestApp(): Promise<TestApp> {
  ...
  const moduleRef = await Test.createTestingModule({
    imports: [SharedKernelModule, LoggerModule, SupportModule],
  }).compile()
...
function resolveFromBearerClaims(
  ...
  const guest = { tenantId: null, slug: 'test-support-guest', chainId: null, isNeutral: true }
