// Supertest-harness UsersAdminController на реальных Postgres/Redis, копия приёма tenants-test-app.ts.
...
import { UsersAdminController } from '@/modules/admin/presentation/users-admin.controller.js'
import { IDENTITY_FACADE_PORT_PROVIDER } from '@/modules/admin/infrastructure/adapters/identity-facade.adapter.js'
import { ListUsersUseCase } from '@/modules/admin/application/use-cases/list-users.use-case.js'
import { DeactivateUserUseCase } from '@/modules/admin/application/use-cases/deactivate-user.use-case.js'
import { ChangeStaffRoleUseCase } from '@/modules/admin/application/use-cases/change-staff-role.use-case.js'
import { GrantPlatformRoleUseCase } from '@/modules/admin/application/use-cases/grant-platform-role.use-case.js'
...
const TEST_ENV: Readonly<Record<string, string>> = {
  ...
  OTP_RATE_LIMIT_KEY_PREFIX: 'test:otp_rl_admin_users',
...
@Module({
  ...
  providers: [IDENTITY_FACADE_PORT_PROVIDER, ListUsersUseCase, DeactivateUserUseCase, ChangeStaffRoleUseCase, GrantPlatformRoleUseCase],
})
// eslint-disable-next-line @typescript-eslint/no-extraneous-class -- модуль тест-харнесса
class UsersAdminTestModule {}
...
export async function createTestApp(): Promise<TestApp> {
  ...
  const moduleRef = await Test.createTestingModule({
    imports: [SharedKernelModule, LoggerModule, AuthModule, AuditLogModule, UsersAdminTestModule],
  }).compile()
...
// TenantContext резолвится из claims.tenantId Bearer-токена — нужен guard'ам.
...
function resolveFromBearerClaims(
  ...
  const guest = { tenantId: null, slug: 'test-admin-users-guest', chainId: null, isNeutral: true }
