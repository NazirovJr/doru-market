// Supertest-harness AuditLogController на реальных Postgres/Redis, копия приёма tenants-test-app.ts.
...
const TEST_ENV: Readonly<Record<string, string>> = {
  ...
  OTP_RATE_LIMIT_KEY_PREFIX: 'test:otp_rl_admin_audit_log',
  ...
  JWT_KID: 'test-v1-admin-audit-log',
...
export async function createTestApp(): Promise<TestApp> {
  ...
  const moduleRef = await Test.createTestingModule({
    imports: [SharedKernelModule, LoggerModule, AuthModule, AuditLogModule],
  }).compile()
...
// TenantContext резолвится из claims.tenantId Bearer-токена — нужен AuthGuard.assertCrossTenantAccess.
...
function resolveFromBearerClaims(
  ...
  const guest = { tenantId: null, slug: 'test-admin-audit-log-guest', chainId: null, isNeutral: true }
  ...
  return {
    ...
    slug: 'test-admin-audit-log-guest',
