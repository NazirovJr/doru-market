 ...
 *
 * `tenantId === null` означает «тенант НЕ РЕЗОЛВЛЕН» (`unresolved: true`,
 * см. `forUnresolved()`). Нейтральный тенант — это НАСТОЯЩАЯ строка в таблице
 * `tenants` с настоящим UUID: для успешно резолвленного нейтрального случая
 * `tenantId` содержит этот UUID, а различие «нейтральный / обычный» несёт
 * отдельное поле `isNeutral`. `tenantId` и `isNeutral` — независимые оси:
 * не пытайтесь выводить одно из другого. Это различие важно для guard'а
 * DTJ-055 и для потребителей, которым `tenantId` нужен как реальный UUID
 * (например, `OtpVerifyController` — поле едет в колонку `UUID NOT NULL`).
 */
