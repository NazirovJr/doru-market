...
describe('ExpiryDate VO (DTJ-011, SRS-DOM-087/088)', () => {
  ...
  it('3. isSellable: товар с expiry 2026-08-27 при today 2026-08-27 → false (включительно)', () => {
    ...
    if (!isOk(exp)) return
    expect(exp.value.isSellable(new Date('2026-08-27T00:00:00.000Z'))).toBe(false)
  })
  ...
  it('4. isSellable: товар с expiry 2026-08-28 при today 2026-08-27 → true (день после)', () => {
    ...
    expect(isOk(exp)).toBe(true)
    if (!isOk(exp)) return
    expect(exp.value.isSellable(new Date('2026-08-27T00:00:00.000Z'))).toBe(true)
  })
  ...
  it('6. hasMinimumRemainingShelfLife: 30 дней запаса при minDays=30 → true', () => {
    ...
    expect(isOk(exp)).toBe(true)
    if (!isOk(exp)) return
    expect(exp.value.hasMinimumRemainingShelfLife(new Date('2026-08-27T00:00:00.000Z'), 30)).toBe(true)
  })
  ...
  it('7. hasMinimumRemainingShelfLife: 29 дней запаса при minDays=30 → false (мягкое правило)', () => {
    ...
    expect(isOk(exp)).toBe(true)
    if (!isOk(exp)) return
    expect(exp.value.hasMinimumRemainingShelfLife(new Date('2026-08-27T00:00:00.000Z'), 30)).toBe(false)
  })
