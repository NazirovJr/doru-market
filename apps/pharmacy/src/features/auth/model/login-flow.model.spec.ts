...
/**
 * Портировано из `apps/web/src/features/auth/model/login-flow.model.spec.ts` (DTJ-028) — reducer
 * идентичен (см. JSDoc `login-flow.model.ts`), тесты переносятся без изменений логики проверок.
 */
describe('login-flow.model (DTJ-166)', () => {
  describe('reducer', () => {
    ...
    it('2. phoneSubmit: phone → code, resendCooldownSeconds=60', () => {
      ...
      expect(next.phone).toBe('+992937001122')
    ...
    it('7b. goBackToPhone используется LoginPage.tsx для отказа роли (критерий приёмки 4) — тот же сброс', () => {
      const inCode: LoginFlowState = {
        ...initialState,
        step: 'code',
        otpRequestId: 'req-1',
      ...
      const next = loginFlowReducer(inCode, { type: 'goBackToPhone' })
      expect(next.step).toBe('phone')
      expect(next.otpRequestId).toBeNull()
    })
